...
public partial class RegistoProducaoViewModel : ObservableObject
{
    ...
    public async Task LoadAsync(ProducaoPecaDisponivelItem? requestedContext)
    {
        ...
        try
        {
            ...
            var gestorProducao = await gestorProducaoTask;
            var fases = await fasesTask;
            var maquinas = await maquinasTask;
            var registos = await registosTask;

            GestorProducaoId = gestorProducao?.User_id;
            GestorProducaoNome = gestorProducao?.Nome ?? string.Empty;
            _todasFases = fases;
            _todasMaquinas = maquinas;
            _todosRegistos = registos;
